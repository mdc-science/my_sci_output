...
fig_cumdocs <- 
  ...
  labs(title = "Published documents, cumulative - All types",
...
# Tidying data
dt_publisher <- count(df_papers, publisher)
dt_publisher <- arrange(dt_publisher, desc(n))
dt_publisher <- dt_publisher %>% mutate(fill_map = as.factor(row_number()))
...
fig_alldocs_publisher <- 
  ...
  labs(title = "Published documents - Publisher",
       subtitle = paste0("Period: ", period_min,"-", period_max,
                         " (updated on ", last_updated, ")")) +
  xlab("Publisher") +
  ylab("Documents") + 
...
# Tidying data
dt_issue_type <- count(df_papers, issue_type)
dt_issue_type <- arrange(dt_issue_type, n)
dt_issue_type <- dt_issue_type %>% mutate(fill_map = as.factor(row_number()))

issue_type_label <- c("special_issue" = "Special Issue", "regular_issue" = "Regular Issue", 
                      "book" = "Book")
...
fig_alldocs_issue_type <- 
  ...
  scale_fill_brewer(palette = "Spectral") +
  labs(title = "Published documents - Issue type",
       subtitle = paste0("Period: ", period_min,"-", period_max,
                         " (updated on ", last_updated, ")")) +
...
# Tidying data
dt_type <- count(df_papers, type)
dt_type <- arrange(dt_type, desc(n))
dt_type <- dt_type %>% mutate(fill_map = as.factor(row_number()))

doc_type_label <-  c("article" = "Original\nResearch", "editorial" = "Editorial", 
...
fig_alldocs_type <- 
  ...
  labs(title = "Published documents - Article type",
       subtitle = paste0("Period: ", period_min,"-", period_max, " (updated on ", last_updated, ")")) +
  xlab("Document type") +
  ylab("Documents") + 
  theme_few() + 
...
dt_qualis <- arrange(dt_qualis, qualis_2017_2020)
dt_qualis <- dt_qualis %>% mutate(fill_map = as.factor(row_number()))
...
fig_qualis <- 
  ...
  geom_text(aes(label = n), nudge_y = 1.5) +
  scale_fill_brewer(palette = "Spectral") +
  labs(title = "Published documents - Qualis CAPES",
       subtitle = paste0("Period: ", period_min,"-", period_max, " (updated on ", last_updated, ")")) +
...
#### Number of publications in per Journal with IF_2021 ####
# Tidying data
dt_journal <- count(df_all, journal_abbr)
dt_journal <- arrange(dt_journal, n)
dt_journal <- merge(dt_journal, df_journals, by = "journal_abbr")
...
fig_alldocs_journal <- 
  ...
  xlab("Documents") +
  ylab("") + 
  scale_x_continuous(limits = c(0, max(dt_journal$n, na.rm = T) + 2), 
...
# Splitting multiple correspoding authors
df_corrauth <- df_all %>% 
  select(year, title, corresp_auth) %>% 
  separate_wider_delim(corresp_auth, delim = ";", too_few = "align_start",
...
fig_corrauth <- 
  ...
  labs(title = "Published documents - Corresponding author",
       subtitle = paste0("Period: ", period_min,"-", period_max,
                         " (updated on ", last_updated, ")")) +
  xlab("Documents") +
  ylab("") + 
...
fig_citations_wos <- 
  ...
  scale_x_continuous(limits = c(0, round_any(max(df_all$citations_wos, na.rm = T), 10, f = ceiling)), 
  ...
  labs(title = "Citations, all bases - Web of Science",
...
fig_citations_scopus <- 
  ...
  geom_vline(xintercept = seq(0, round_any(max(df_all$citations_scopus, na.rm = T), 20, f = ceiling), by = 10), 
             linetype = "dashed", color = "gray", alpha = 0.5) + 
  geom_col(aes(fill = citations_scopus), color = "gray25") +
  scale_fill_gradient(low = "ivory", high = "#f16913") +
  scale_x_continuous(limits = c(0, round_any(max(df_all$citations_scopus, na.rm = T), 20, f = ceiling)), 
...
# Find all objects that are plots and start with 'fig_'
