...
table1_theme <-
  ttheme(
    tbody.style = tbody_style(
      color = "black", fill = c("#f7fbff", "#deebf7"),
...
tb_table1 <- tb_table1 %>%
  ...
  tab_add_footnote(
    text = paste0(
      "SCImago Q1: ", round((sum(df_all$quartile_2023 == "Q1", na.rm = T) / nrow(df_all)) * 100, digits = 2), "%",
...
tb_table1 <- tb_table1 %>%
  tab_add_hline(
    at.row = nrow(df_table1) + 3, row.side = "bottom",
    linetype = 1,
    linewidth = 2,
    linecolor = "#08519c"
  )
...
write_csv(df_table1, here("table", paste0(format(Sys.Date(), "%Y.%m.%d"), "_", period_min, "-", period_max, "_sum_tb_scimago_quartile.csv")))

ggsave(
  file = here("table", paste0(format(Sys.Date(), "%Y.%m.%d"), "_", period_min, "-", period_max, "_sum_tb_scimago_quartile.png")),
  tb_table1, bg = "white",
  height = 32, width = 23, units = "cm", dpi = 600
)
