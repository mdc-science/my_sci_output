...
tb_table2 <- tb_table2 %>%
  ...
  tab_add_footnote(text = paste0("Qualis A1: ", round((sum(df_all$qualis_2017_2020 == "A1", na.rm = T)/nrow(df_all))*100, digits = 2), "%",
...
tb_table2 <- tb_table2 %>% 
  tab_add_hline(at.row = nrow(df_table2) + 3, row.side = "bottom", 
                linetype = 1,
                linewidth = 2,
                linecolor = "#a50f15")
